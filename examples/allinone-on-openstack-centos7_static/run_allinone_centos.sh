...
cat << EOF > /etc/yum.repos.d/CentOS-Vault-current.repo
[C7.2.1511-updates]
...
sed -i "s/bridged_ip/${IP}/" examples/allinone-on-openstack-centos7_static/params.pp

# get the network of the bridged interface and replace some variables
NETWORK=$(ip r | grep -v default | grep -e '^192.168.0\.' | cut -d' ' -f1)
sed -i "s%bridged_network%${NETWORK}%" examples/allinone-on-openstack-centos7_static/params.pp

ALLOWED_HOST_NETWORK=$(ip r | grep -v default | grep -e '^192.168.0\.' | cut -d' ' -f1 | cut -d'/' -f1 | cut -d'.' -f1,2,3).%
sed -i "s,allowed_host_network,${ALLOWED_HOST_NETWORK}," examples/allinone-on-openstack-centos7_static/params.pp

#Override the params.pp
cp examples/allinone-on-openstack-centos7_static/params.pp manifests/params.pp
...
puppet apply -e "class {'::midonet_openstack::role::allinone_static': zk_id => 1 }" --debug --trace 2>&1 | tee /tmp/puppet-$(date +"%Y-%m-%d_%H-%M-%S").out
